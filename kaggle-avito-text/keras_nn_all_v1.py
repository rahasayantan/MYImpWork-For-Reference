...
def nn_model4():
    ...
    model.compile(loss = ["MSE"], metrics=[root_mean_squared_error], optimizer = ks.optimizers.Adam(lr=0.00001, epsilon=1e-08, decay=0.))
    return(model)
    ...
    for seed in [2018]:
        ...
        for dev_index, val_index in kf.split(y):
            ...
            tstpreds = model.predict(get_keras_sparse(test_df.values, 
                                                    ...
                                                    ts_val, pa_val, desc_val,
                                                    re_test, ci_test, ca_test, pca_test, utc_test, p1_test, p2_test, p3_test
                                                    )).reshape(-1,1)
            print("predicting..")
            ...
            model =load_model("./weights3XXLK.hdf5",custom_objects={'root_mean_squared_error': root_mean_squared_error})
...
joblib.dump(oobpred1,'../input/train_keras_all.pkl')
joblib.dump(pred1,'../input/test_keras_all.pkl')

#####################

preds = (oobtest[:,0] + oobtest[:, 1])/2
sub = pd.DataFrame(preds,columns=["deal_probability"])
sub['item_id'] = item_id
sub['deal_probability'].clip(0.0, 1.0, inplace=True) # Between 0 and 1
sub.to_csv("../output/keras_all_start01.csv",index=False,header=True)
