...
joblib.dump(train, '../input/6/train_nn_ts_2d_smallHr_ds.pkl')
joblib.dump(test, '../input/6/test_nn_ts_2d_smallHr_ds.pkl')
...
def mix_generator(x, batch_size, swaprate=0.1, shuffle=True):
...
print ('autoenc neural network....')
def get_DAE():
    # denoising autoencoder
    inputs = Input((25,), name = 'input_1')
    x = Dense(10, activation='linear', name="l1")(inputs) # 1500 
    x = Dense(10, activation='linear', name="l2")(x) # 1500 
    x = Dense(10, activation='linear', name="l3")(x) # 1500 
    outputs = Dense(25, activation='linear', name="l4")(x)
    model = Model(inputs=inputs, outputs=outputs)
    optimizer_adam = Adam(lr=0.001, decay = 1e-5)
    model.compile(loss='mse',optimizer=optimizer_adam)
    return model

batch_size = 128
num_epoch = 1
gen = mix_generator(train[cols2scale].values, batch_size)
dae = get_DAE()
dae.summary()
save_checkpoint = ModelCheckpoint(filepath='save_dae.h5', monitor='loss', save_best_only=True)
...
def get_keras_data(dataset1, y, batch_size = 64, shuffle=False):
    ...
    while True:
        ...
        X = {
            ...
            ,'input_1': batch_x[cols2scale].values
        }
...
def nn(dae):
    ...
    l2_loss = l2(0.05)
    dae.trainable = False
    daeinputs = dae.inputs
    #daeinputs = Input(shape=[25], name = 'autoenc')
    #x1 = dae.get_layer('l1').output
    x = concatenate([dae.get_layer('l1').output, dae.get_layer('l2').output, dae.get_layer('l3').output])#(daeinputs)
    x = Dropout(0.2)(x)
    x = Dense(30, activation='relu', kernel_regularizer=l2_loss)(x)
    ...
    z = Dropout(0.2)(Dense(150,activation='relu')(s_dout))#(s_dout))#
    z = Dropout(0.2)(Dense(150,activation='relu')(z))
    z = Dropout(0.2)(Dense(100,activation='relu')(z))
...
#            val_y2 = val_y[:1000000]
            ...
            model = nn(dae)
            model.summary()
            earlystopper = EarlyStopping(monitor='val_loss', patience=2, verbose=1 , mode='auto')
            plateau = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=1, verbose=0, mode='auto', epsilon=0.0001, cooldown=0, min_lr=0)
            save_model_path = "nn_sav_"+str(i) +".h5"
            checkpointer = ModelCheckpoint(save_model_path, verbose=1, save_best_only=True)
            optimizer_adam = Adam(lr=0.001, decay=lr_decay)
            model.compile(loss='binary_crossentropy',optimizer=optimizer_adam,metrics=['accuracy'])#loss_weights=[1., 0.1]
...
oobpred = oobval / nbag
joblib.dump(oobpred,'../input/6/train_keras_embed_red_ds_3f.pkl')
joblib.dump(pred,'../input/6/test_keras_embed_red_ds_3f.pkl')
