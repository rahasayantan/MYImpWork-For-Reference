...
for i, (train_index, valid_index) in enumerate(kfold.split(train_XX1, train_y)):
    ...
    model.fit([X_train], Y_train, batch_size=256, epochs=10, validation_data=([X_val], Y_val), #class_weight=class_weight,
    verbose=2, callbacks=callbacks)
    model.load_weights(filepath)
    y_pred = model.predict([X_val], batch_size=1024, verbose=2)
    y_test += np.squeeze(model.predict([test_XX1,test_XX1], batch_size=1024, verbose=1))/5
